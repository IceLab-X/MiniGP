# ---- coding: utf-8 ----
# @author: Ziyang Zhang
# @version: v2, add more kernels (matern 3 and 5)
#  - observationGP:    x->y, matern3+5 (or RBF)
...
class GPDM(torch.nn.Module):
    def __init__(self, D, Q, dyn_target):
        ...
        """ 
        NOTE: You should define TWO separate parameter sets since we have oGP and dGP
        """
    ...
    def get_x_neg_log_likelihood(self, Xout, Xin, flg_W=True):
        ...
        L_x = Q/2*log(|K_x(Xin,Xin)|) + 1/2*trace(K_x^-1*Xout*W_x^2*Xout) - N*log(|W_x|)

        If use original GPDM, choose flg_W = False
        L_x = Q/2*log(|K_x(Xin,Xin)|) + 1/2*trace(K_x^-1*Xout*Xout)
...
if __name__ == "__main__":
    ...
    """ hyper-parameters """
    Q = 3        # latent dim
    epochs = 10  # optimization steps (max epochs)
    ...
    plt.axvline(x=N, linestyle='dashed', color='grey')
    plt.show()

    # plot all the 5 dimensions
    plt.figure()
