...
if __name__ == '__main__':
    ...
    # test data
    x1 = torch.linspace(0, 5, 101).unsqueeze(1)
    x2 = torch.linspace(0, 5, 101).unsqueeze(1)

    input_dim = 1
    # test kernel
    test_kernel = LinearKernel(input_dim)
    test_kernel = ARDKernel(input_dim)
    # test_kernel = MaternKernel(input_dim)
    # #
    # test_kernel = SquaredExponentialKernel()
    # test_kernel = RationalQuadraticKernel()
    # test_kernel = MaternKernel_scalarLengthScale()

    # #test kernel operation
    # kernel1 = LinearKernel(input_dim)
    # kernel2 = ARDKernel(input_dim)
    # #
    # test_kernel = SumKernel(kernel1,kernel2)
    # test_kernel = ProductKernel(kernel1,kernel2)


    kernel_matrix = test_kernel(x1, x2)

    import matplotlib.pyplot as plt
    plt.imshow(kernel_matrix.detach().numpy(), cmap='viridis', origin='lower')
