...
class cigp(nn.Module):
    ...
    def kernel(self, X1, X2):
        ...
        K = (
            -2.0 * X1 @ X2.t() + X1_norm2.expand(X1.size(0), X2.size(0)) + X2_norm2.t().expand(X1.size(0), X2.size(0))
    ...
    def kernel_matern3(self, x1, x2):
        ...
        distance = const_sqrt_3 * torch.cdist(x1, x2, p=2).to(device)
    ...
    def forward(self, Xte):
        ...
        Sigma = (
            ...
            + self.log_beta.exp().pow(-1) * torch.eye(self.X.size(0)).to(device)
            + JITTER * torch.eye(self.X.size(0)).to(device)
        )
...
if __name__ == "__main__":
    ...
    # xte = torch.linspace(0, 6, 100).view(-1, 1).to(device)
    # yte = (torch.sin(xte) + 10).to(device)

    # xtr = (torch.rand(16, 1) * 6).to(device)
    # ytr = torch.sin(xtr) + (torch.randn(16, 1) * 0.5 + 10).to(device)
